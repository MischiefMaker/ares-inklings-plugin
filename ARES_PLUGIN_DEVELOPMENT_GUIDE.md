...
### Lesson 23: App-Review Integration Hook — Pattern for Extensible Staff Views

When a plugin needs to display supplementary information on the MUSH character app-review
screen (`/char/app <charname>`), use an official hook so multiple plugins can contribute
without colliding.

**Pattern:** Define a hook in Chargen (or the relevant core plugin):
```ruby
def self.get_app_review_details(char)
  details = []
  # Plugins add their own review info via this hook
  AresMUSH.plugin_manager.dispatch_plugin_message(
    "get_app_review_details",
    char
  )
  return details
end
...
**Plugin side** (Inklings example):
```ruby
def self.get_app_review_details(char)
  return Inklings.get_app_review_issues(char)
end
```

**Benefits:**
- Multiple plugins can each contribute their own app-review details
- MUSH command displays all details inline without switching to web portal
- Extensible pattern: SOUL, Marque, or future plugins can hook in the same way
- Staff get a complete picture on the command line

**Key points:**
- Hook name should be generic (`get_app_review_details`) not plugin-specific
- Return format should match the consuming screen's display (array of status strings)
- Plugins implement the hook in their main module, not in Chargen itself
- Document the hook in the core plugin so plugin authors know it exists

**When to use:**
- A plugin has information staff needs to see during character approval
- Multiple plugins might each have app-review information
- Avoiding multiple hops between MUSH and web portal is a staff workflow win

This pattern is extensible for any "plugin contributes detail to a staff view" situation,
not just app-review. Use it for any core plugin's hook that other plugins need to extend.
